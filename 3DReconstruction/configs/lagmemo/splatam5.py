...
group_name = "5cdEh9F2hJL_1"
run_name = f"{seed}"

config = dict(
    workdir=f"./data_for_gsmodel/{group_name}/{mapping_iters}",
    run_name=run_name,
    seed=seed,
    ...
    checkpoint_time_idx=50,
    save_checkpoints=False,  # Save Checkpoints
    checkpoint_interval=50,  # Checkpoint Interval
    use_wandb=False,
    eval_checkpoint=False,  # 暂时用来试着eval一下
    wandb=dict(
        entity="",
        project="",
        group=group_name,
    ...
    data=dict(
        basedir="/home/sfs/data_for_gsmodel/5cdEh9F2hJL_1",
        gradslam_data_cfg="./configs/data/lagmemo5.yaml",
        sequence=scene_name,
        desired_image_height_init=720,
        desired_image_width_init=1080,
        desired_image_height=720,
        desired_image_width=1080,
        start=0,
        end=-1,
        stride=1,
        num_frames=-1,
