...
def myRenderer(dataset, params, current_idx, render_idxes, dir):
    ...
    for idx in render_idxes:
        # Get RGB-D Data & Camera Parameters
        color, depth, intrinsics, pose = dataset[idx]
        # print(pose)
        ...
        # Get current frame Gaussians
        transformed_gaussians = transform_to_frame(params, idx, 
                                                   gaussians_grad=False, 
                                                   camera_grad=False)
 
        # Define current frame data
        curr_data = {'cam': cam, 'im': color, 'depth': depth, 'id': idx, 'intrinsics': intrinsics, 'w2c': first_frame_w2c}
        ...
        psnr = calc_psnr(im, curr_data['im']).mean()
        psnr = psnr.detach().cpu().numpy().item()
        ...
        render_rgb_dir = os.path.join(dir, f"iter{current_idx}")
        os.makedirs(render_rgb_dir, exist_ok=True)

        # cv2.imwrite(os.path.join(render_rgb_dir, "gs_{:04d}.png".format(idx)), cv2.cvtColor(viz_render_im*255, cv2.COLOR_RGB2BGR))  
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        import pdb
        # pdb.set_trace()
        # print("这是GaussianRasterizer定义和forward接受输入的位置")
        depth_sil, _, _, = Renderer(raster_settings=curr_data['cam'])(**depth_sil_rendervar)
        rastered_depth = depth_sil[0, :, :].unsqueeze(0)
        ...
        try:
            ...
            # print("No PSNR, SSIM, LPIPS Calculation")
            pass
