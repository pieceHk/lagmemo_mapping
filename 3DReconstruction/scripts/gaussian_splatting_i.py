...
from utils.common_utils import seed_everything, save_params, save_params_pth
...
def get_dataset(config_dict, basedir, sequence, **kwargs):
    ...
    elif config_dict["dataset_name"].lower() in ["lagmemo"]:
...
def get_pointcloud(
    ...
    transform_matrix = torch.tensor([[1, 0, 0], [0, -1, 0], [0, 0, -1]]).cuda().float()
    pts_cam = torch.matmul(pts_cam, transform_matrix)
...
def offline_splatting(config: dict, scene_path=""):
    # Print Config
    print("Loaded Config:")
    ...
    loaded_params = None
    if scene_path != "":
        loaded_params = np.load(scene_path)
        useful_keys = [
            "means3D",
            "rgb_colors",
            "unnorm_rotations",
        ...
        params = {key: loaded_params[key] for key in useful_keys}
        for k, v in params.items():
            if not isinstance(v, torch.Tensor):
                params[k] = torch.nn.Parameter(
                    torch.tensor(v).cuda().float().contiguous().requires_grad_(True)
    ...
    if config["use_wandb"]:
        ...
        wandb_run.define_metric(
            "Number of Gaussians - Densification", step_metric="Mapping_Iters"
        )
    ...
    init_params, variables, optimizer, intrinsics, w2c, cam = initialize_first_timestep(
        dataset,
        num_frames,
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        if config["use_wandb"]:
            wandb_run.log(
                {"Init/Number of Gaussians": post_num_pts, "Init/step": wandb_time_step}
            )

        # Reset Optimizer & Learning Rates for Full Map Optimization
        optimizer = initialize_optimizer(params, config["train"]["lrs_mapping"])
        means3D_scheduler = get_expon_lr_func(
            lr_init=config["train"]["lrs_mapping"]["means3D"],
        ...
        if (time_idx + 1) == num_frames:
            ...
            for iter in range(num_iters_mapping):
                ...
                updated_lr = update_learning_rate(
                    optimizer, means3D_scheduler, iter + 1
                )
                if config["use_wandb"]:
                ...
                iter_data = {
                    ...
                    "w2c": gt_w2c_all_frames_map[iter_time_idx],
                ...
                with torch.no_grad():
                    ...
                    if config["train"]["use_gaussian_splatting_densification"]:
                        ...
                        if config["use_wandb"]:
                            wandb_run.log(
                                {
                                    "Number of Gaussians - Densification": params[
                                        "means3D"
                                    ].shape[0]
                                }
                            )
                    ...
                    # if (iter + 1) % 5000 == 0 and (iter + 1) != 30000:
                    if (iter + 1) == 7000:
                        print("Evaluating Params at times of 5K Iterations")
                        eval_params = convert_params_to_store(params)
                        output_dir = os.path.join(config["workdir"], config["run_name"])
                        eval_dir = os.path.join(output_dir, "eval_7k")
                        # eval_dir = os.path.join(output_dir, "eval", f"iter_{iter + 1}")
    ...
    # temp_params = {"gt_w2c_all_frames": []}
    params["gt_w2c_all_frames"] = []
    for gt_w2c_tensor in gt_w2c_all_frames:
        params["gt_w2c_all_frames"].append(gt_w2c_tensor.detach().cpu().numpy())
    params["gt_w2c_all_frames"] = np.stack(params["gt_w2c_all_frames"], axis=0)
    # for gt_w2c_tensor in gt_w2c_all_frames:
    #    temp_params["gt_w2c_all_frames"].append(gt_w2c_tensor.detach().cpu().numpy())
    # temp_params["gt_w2c_all_frames"] = np.stack(
    #    temp_params["gt_w2c_all_frames"], axis=0
    # )
    # if scene_path != "":
    #    params["gt_w2c_all_frames"] = loaded_params["gt_w2c_all_frames"]
    #    params["gt_w2c_all_frames"] = np.concatenate(
    #        (params["gt_w2c_all_frames"], temp_params["gt_w2c_all_frames"]), axis=0
    #    )
    # else:
    #    params["gt_w2c_all_frames"] = temp_params["gt_w2c_all_frames"]

    # if scene_path != "":
    #    temp_params = {
    #        key: loaded_params[key] for key in ["cam_unnorm_rots", "cam_trans"]
    #    }
    #    params["cam_unnorm_rots"] = np.concatenate(
    #        (temp_params["cam_unnorm_rots"], params["cam_unnorm_rots"]), axis=2
    #    )
    #    params["cam_trans"] = np.concatenate(
    #        (temp_params["cam_trans"], params["cam_unnorm_rots"]), axis=2
    #    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scene", type=str, default="", help="Path to the scene params"
    )
    ...
    offline_splatting(experiment.config, scene_path)
