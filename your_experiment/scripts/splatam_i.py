...
_PROJECT_ROOT = os.path.dirname(_BASE_DIR)  # This should be the 3DGS directory
...
from utils.eval_helpers import report_loss, report_progress, eval, myRenderer
...
def setup_logging_from_config(config):
    ...
    不重定向 `sys.stdout`/`sys.stderr` 到 logging，避免 tqdm 的行内更新被重复记录到日志文件中。
    """
    if 'logdir' in config:
        ...
        # Ensure the stream handler writes to the original stdout to avoid recursion
        ...
        # Create a tee that writes to both the original stdout and the log file,
        # but only writes to the file when a newline is encountered. That keeps
        # the log file synchronized with terminal output while avoiding many
        # intermediate tqdm carriage-return updates being appended.
        ...
        class StreamTee:
            ...
            def write(self, buf):
                try:
                    # Always write to original stream (console)
                    self.orig.write(buf)
                    self.orig.flush()
                except Exception:
                    pass
                # Buffer and only write complete lines to the file to avoid
                # capturing carriage-return based progress updates.
                self._buffer += buf
                while '\n' in self._buffer:
        ...
        # Replace stdout/stderr with the tee so prints go to both console and file.
        sys.stdout = StreamTee(sys.__stdout__, log_file)
        sys.stderr = StreamTee(sys.__stderr__, log_file)
...
def get_dataset(config_dict, basedir, sequence, **kwargs):
    name = config_dict.get("dataset_name", "").lower()
    if name in ["icl"]:
        return ICLDataset(config_dict, basedir, sequence, **kwargs)
    elif name in ["replica"]:
        return ReplicaDataset(config_dict, basedir, sequence, **kwargs)
    elif name in ["replicav2"]:
        return ReplicaV2Dataset(config_dict, basedir, sequence, **kwargs)
    elif name in ["azure", "azurekinect"]:
        return AzureKinectDataset(config_dict, basedir, sequence, **kwargs)
    elif name in ["scannet"]:
        return ScannetDataset(config_dict, basedir, sequence, **kwargs)
    elif name in ["ai2thor"]:
        return Ai2thorDataset(config_dict, basedir, sequence, **kwargs)
    elif name in ["record3d"]:
        return Record3DDataset(config_dict, basedir, sequence, **kwargs)
    elif name in ["realsense"]:
        return RealsenseDataset(config_dict, basedir, sequence, **kwargs)
    elif name in ["tum"]:
        return TUMDataset(config_dict, basedir, sequence, **kwargs)
    elif name in ["scannetpp"]:
        return ScannetPPDataset(basedir, sequence, **kwargs)
    elif name in ["nerfcapture"]:
        return NeRFCaptureDataset(basedir, sequence, **kwargs)
    elif name in ["lagmemo"]:
        return LagmemoDataset(config_dict, basedir, sequence, **kwargs)
    else:
        raise ValueError(f"Unknown dataset name {config_dict.get('dataset_name')}")
...
def initialize_params(init_pt_cld, num_frames, mean3_sq_dist, gaussian_distribution):
    ...
    cam_rots = np.tile(cam_rots[:, :, None], (1, 1, MAX_NUM_FRAME))
    params["cam_unnorm_rots"] = cam_rots
    params["cam_trans"] = np.zeros((1, 3, MAX_NUM_FRAME))
...
def rgbd_slam(config: dict):
    ...
    logger.info("Starting RGBD SLAM...")
    ...
    if config["load_checkpoint"]:
        ...
        ckpt_eval_dir = os.path.join(eval_dir, str(checkpoint_time_idx))
    ...
    for time_idx in range(checkpoint_time_idx, num_frames):
        ...
        # Only show tqdm at the end of mapping step
        progress_bar = None
        ...
        if time_idx == 0 or (time_idx + 1) % config["map_every"] == 0:
            ...
            with torch.no_grad():
                ...
                # num_keyframes = config["mapping_window_size"]
            ...
            # When the scene gets larger, more iterations are needed
            num_iters_mapping = max(config["mapping"]["num_iters"], time_idx // 10)
            if num_iters_mapping > 0:
                progress_bar = tqdm(
                    range(num_iters_mapping), desc=f"Mapping Time Step: {time_idx}",
                    mininterval=1000,  # effectively disables intermediate refresh
                    file=sys.__stdout__  # force output to real stdout
                )
            for iter in range(num_iters_mapping):
                ...
                # sfs modified
                # before nums // 2, use current frame data
                # after nums // 2, select according to psnr_dict
                if iter < config["mapping"]["num_iters"] // 3:
                    selected_rand_keyframe_idx = -1
                else:
                    if len(psnr_dict) < 2:
                        selected_rand_keyframe_idx = np.random.randint(0, max(1, time_idx))
                    else:
                        psnr_values = np.array(list(psnr_dict.values()))    
                        weights = 1.0 / (psnr_values + 1e-6)
                        weights = weights ** 4  # Exponentially increase the weight
                        weights = weights / np.sum(weights)         
                        selected_rand_keyframe_idx = np.random.choice(
                            list(psnr_dict.keys()), p=weights
                        )
            ...
            if num_iters_mapping > 0:
                ...
                print(f"[GPU Memory] Mapping Time Step: {time_idx} | Allocated: {mem_mb:.2f} MB | Max: {max_mem_mb:.2f} MB")
            # Update the runtime numbers
        ...
        # sfs: myRenderer
        if time_idx % 5 == 0:
            frame_indices = list(range(time_idx + 1))
            psnr_dict = myRenderer(dataset, params, time_idx, frame_indices, eval_dir)
    ...
    save_w2c(params, output_dir)
    save_params_pth(params, variables, output_dir)
