...
class GaussianModel:
    ...
    def restore(self, model_args, training_args, mode="train"):
        if len(model_args) == 13:  # 这是一个feature训练时保存的ckpt
            ...
            ):  # 如果是以原始gs为初始化来训练feature的话，就不需要restore optimizer
    ...
    def training_setup(self, training_args):
        ...
        if training_args.include_feature:
            ...
            assert (
                self._language_feature is None
            ), "在训练原始gs的时候language feature应该始终为None"

        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)
        # zht schedular_args根本没用
        # self.xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init*self.spatial_lr_scale,
        #                                             lr_final=training_args.position_lr_final*self.spatial_lr_scale,
        #                                             lr_delay_mult=training_args.position_lr_delay_mult,
        #                                             max_steps=training_args.position_lr_max_steps)

    # zht update_learning_rate根本没用
