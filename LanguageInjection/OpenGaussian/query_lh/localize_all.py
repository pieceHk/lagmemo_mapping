...
if __name__ == "__main__":
    ...
    # 选择top k
    k = 5
    output_path = f"./output_new/sam_level_3/tabs_and_figs_{model_name.split('/')[-1]}"
    ...
    goal_dir = "/home/sfs/OpenGaussian/query_lh/groundtruth_data_new/groundtruth_data_tee_new"
    ...
    with Tee(output_txt) as t:
        for task_dir in task_dirs: # task_dir "00display cabinet"
            ...
            if args.output_ply:
                ...
                ply_output_path = f"./localize_vis_new/sam_level_3/{model_name.split('/')[-1]}"
        ...
        # plt.figure(figsize=(12, 8))
        # ax = plt.gca()
        # scatter = ax.scatter(all_occu_counts, all_distances, alpha=0.6)
        # add_labels(all_occu_counts, all_distances, ax)
        # plt.xlabel("Occupancy Count")
        # plt.ylabel("Distance to GT (m)")
        # plt.title("Point Density vs Localization Accuracy (Number indicates image index)")
        # plt.savefig(os.path.join(output_path, "density_vs_accuracy_labeled.png"))
        # plt.close()

        # # 距离分布直方图
        # plt.figure(figsize=(10, 6))
        # plt.hist(valid_distances, bins=20, edgecolor='k')
