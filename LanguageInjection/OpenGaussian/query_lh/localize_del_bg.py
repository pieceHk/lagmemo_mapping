...
def filter_background_regions(leaf_lang_feat, open_clip_network, threshold=0.3):
    background_words = ["wall", "floor", "ceiling", "door", "window"]
    with torch.no_grad():
        bg_features = [F.normalize(open_clip_network.encode_text([w]).float(), dim=-1) for w in background_words]
        bg_features = torch.cat(bg_features, dim=0).to(leaf_lang_feat.device)
        bg_sim = torch.matmul(leaf_lang_feat, bg_features.T)  # [num_leaf, num_bg]
        bg_max_sim = bg_sim.max(dim=1).values

         # 绘图显示背景相似度分布
        plt.figure(figsize=(10, 6))
        plt.hist(bg_max_sim.cpu().numpy(), bins=50, color='skyblue', edgecolor='black')
        plt.axvline(threshold, color='red', linestyle='--', label=f'Threshold = {threshold}')
        plt.title("Background Similarity Distribution")
        plt.xlabel("Max Similarity with Background Words")
        plt.ylabel("Number of Regions")
        plt.legend()
        plt.tight_layout()
        plt.savefig("/home/sfs/OpenGaussian/query_lh/bg_max_sim_hist.png")
        plt.close()

        mask = bg_max_sim > threshold
        leaf_lang_feat[mask] = 0.0
        print(f"Filtered {mask.sum().item()} regions due to background similarity > {threshold}")
...
if __name__ == "__main__":
    ...
    gt_pose_path = "/home/sfs/4_new_lagmemo_scenes/record_data_for4rooms_v2/VBz/0/local_pos.txt"
    ...
    model_name = "VBz_32_5_30_60"
    # 选择top k
    k = 5
    output_path = f"/home/sfs/OpenGaussian/query_lh/experiment_new3/output/sam_level_3/tabs_and_figs_{model_name.split('/')[-1]}"
    ...
    filter_background_regions(leaf_lang_feat, open_clip_network, threshold=0.23)

    _, t_0, R_0_inv = get_first_frame(gt_pose_path)
    goal_dir ="/home/sfs/OpenGaussian/query_lh/groundtruth_data_new3/new_data3/VBzV5z6i1WS"
    ...
    with Tee(output_txt) as t:
        for task_dir in task_dirs: # task_dir "00display cabinet"
            ...
            if args.output_ply:
                ...
                ply_output_path = f"/home/sfs/OpenGaussian/query_lh/experiment_del_bg/localize_vis/sam_level_3/{model_name.split('/')[-1]}"
