...
def get_SAM_mask_and_feat(gt_sam_mask, level=3, filter_th=50, original_mask_feat=None, sample_mask=False):
    ...
    """
    num_levels = int(gt_sam_mask.shape[0])
    if num_levels <= 0:
        raise ValueError("gt_sam_mask has no SAM levels.")

    req_level = int(level)
    level = max(0, min(req_level, num_levels - 1))
    if level != req_level:
        key = (req_level, num_levels)
        if key not in _SAM_LEVEL_WARNED:
            warnings.warn(
                f"Requested sam_level={req_level}, but available levels are [0, {num_levels - 1}]. "
                f"Fallback to sam_level={level}.",
                stacklevel=2,
            )
            _SAM_LEVEL_WARNED.add(key)
