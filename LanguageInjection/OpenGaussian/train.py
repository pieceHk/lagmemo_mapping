...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, \
    ...
    print("save_memory: ", opt.save_memory)
    # # sfs
    ...
    for iteration in range(first_iter, opt.iterations + 1):     
        ...
        # print(f"mask_feat shape: {viewpoint_cam.original_mask_feat.shape}")
        # print(f"mask_feat: {viewpoint_cam.original_mask_feat}")
        
        gt_sam_mask = None
        if viewpoint_cam.original_sam_mask is not None:
            gt_sam_mask = viewpoint_cam.original_sam_mask.cuda()
        invalid_pix = torch.zeros_like(gt_image[0], dtype=torch.bool)
        ...
        if iteration > opt.start_ins_feat_iter:
            if gt_sam_mask is None:
                # Some views may not contain SAM masks. Skip SAM-supervised losses safely.
                if cb_mode is None:
                    no_need_bk = True
            ...
            # NOTE: Freeze the pre-trained Gaussian parameters and only train the instance features.
                ...
                if cb_mode is None:
                    ...
                    if feat_mean_stack.shape[0]!=1:
                        loss_separation = separation_loss(feat_mean_stack, iteration)
                    else:
                        loss_separation = torch.tensor(0.0).cuda()
        ...
        if no_need_bk:
            ...
            if rendered_ins_feat is not None:
                rendered_ins_feat = rendered_ins_feat.detach()
            if rendered_cluster_imgs is not None:
                rendered_cluster_imgs = [img.detach() for img in rendered_cluster_imgs if img is not None]
            if rendered_leaf_cluster_imgs is not None:
                rendered_leaf_cluster_imgs = [leaf.detach() for leaf in rendered_leaf_cluster_imgs if leaf is not None]
            if loss is not None:
                loss = loss.detach()
        else:
            loss.backward()
        ...
        # 显式释放大型中间变量，释放显存
        # del rendered_cluster_imgs
        del rendered_leaf_cluster_imgs
        ...
        with torch.no_grad():
            ...
            # Optimizer step lhchanged
            if iteration < opt.iterations:
                if not no_need_bk:
                    gaussians.optimizer.step()         # 只有非 no_need_bk 情况才更新
...
def construct_pseudo_ins_feat(scene : Scene, renderFunc, renderArgs, 
    ...
    # Clamp sam_level to available levels for current dataset.
    if len(sorted_train_cameras) > 0 and sorted_train_cameras[0].original_sam_mask is not None:
        max_sam_level = int(sorted_train_cameras[0].original_sam_mask.shape[0] - 1)
        req_sam_level = int(sam_level)
        sam_level = max(0, min(req_sam_level, max_sam_level))
        if sam_level != req_sam_level:
            print(f"[WARN] Requested sam_level={req_sam_level}, but current data only supports up to {max_sam_level}. Use sam_level={sam_level}.")
            sys.stdout.flush()
    ...
    for idx, view in enumerate(tqdm(sorted_train_cameras, desc="construt pseudo feat")):
        ...
        # Some views may not have SAM annotations.
        if view.original_sam_mask is None:
            view.pesudo_ins_feat = torch.zeros_like(rendered_ins_feat)
            H, W = rendered_ins_feat.shape[1], rendered_ins_feat.shape[2]
            view.pesudo_mask_bool = torch.zeros((1, H, W), dtype=torch.bool, device=rendered_ins_feat.device)
    ...
    if mode=="leaf":
        ...
        for idx, view in enumerate(tqdm(sorted_train_cameras, desc="render coarse-level cluster")):
            if not view.data_on_gpu:
                view.to_gpu()
            if view.pesudo_mask_bool is None or view.pesudo_mask_bool.sum() == 0:
                if view.data_on_gpu and save_memory:
                    view.to_cpu()
                continue
    ...
    if mode == "lang":
        ...
        for v_id, view in enumerate(sorted_train_cameras):
            if not view.data_on_gpu:
                view.to_gpu()
            if view.original_sam_mask is None or view.original_mask_feat is None:
                if view.data_on_gpu and save_memory:
                    view.to_cpu()
                continue
            # Clamp per-view sam_level to avoid OOB on mixed/partial SAM levels.
            curr_max_level = int(view.original_sam_mask.shape[0] - 1)
            curr_level = max(0, min(int(sam_level), curr_max_level))

            if curr_level == 0:
                strat_id = 0
                end_id = view.original_sam_mask[curr_level].max().to(torch.int64) + 1
            else:
                strat_id = view.original_sam_mask[curr_level-1].max().to(torch.int64) + 1
                end_id = view.original_sam_mask[curr_level].max().to(torch.int64) + 1
